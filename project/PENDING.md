...
| P14 | **O que realmente exige servidor** — usuário quer minimizar dependência de servidor ao máximo: um único node (ex. só o app no celular) deve funcionar 100% standalone. Servidor só deveria existir pro que de fato exige coordenação entre múltiplos nodes — candidato confirmado: gerenciamento de múltiplos dispositivos (Fase 9). Não confirmado ainda: se emitir/validar chave da "Warden API" (P12) usada de fora do device exige servidor, e se hospedar integrações MCP acessíveis de fora exige servidor | `ARCHITECTURE.md`, pedido do usuário 2026-08-02 | 🟠 Média |
