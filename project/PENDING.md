...
| P20 | **Trait `Channel` ainda não existe** — `PHASE.md` (etapa 2.2) previa uma trait `Channel` unificando canais externos; a Fase 2 (Telegram) optou por uma função concreta reutilizável (`warden_bootstrap::handle_turn`) em vez disso, decisão explícita do usuário, já que com um único implementador (Telegram) uma trait não validaria a forma certa da abstração. Revisitar quando o WhatsApp (Fase 3) existir — aí sim dois exemplos reais (bot HTTP long-polling vs sidecar Node+IPC do Baileys) mostram o que de fato é compartilhável | `ARCHITECTURE.md` (decisão da Fase 2), sessão 2026-08-09 | 🟡 Baixa |
